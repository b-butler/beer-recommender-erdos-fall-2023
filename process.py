...
class InteractionMatrixTransformer:
    ...
    def __init__(self, data):
        ...
        self._shape = (self._reviewer_ids.max() + 1, self._beer_ids.max() + 1)
    ...
    def to_zero_one(self, threshold=2.5, review="overall"):
        ...
        interactions = np.where(interactions >= threshold, 1, 0)
        return sp.sparse.csr_matrix(
            (interactions, self._indices), self._shape, copy=True
        )
    ...
    def to_positive_negative(self, threshold=2.5, review="overall"):
        ...
        """
        interactions = self._data["review_" + review].values
        interactions = np.where(interactions >= threshold, 1, -1)
        return sp.sparse.csr_matrix(
            (interactions, self._indices), self._shape, copy=True
        )
