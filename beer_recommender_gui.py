...
beers = cleaned_df.copy().drop(['review_time',
                             'review_overall',
                             'review_aroma',
                             'review_appearance',
                             'review_profilename',
...
beers = beers[['beer_id',
               'beer_name',
               'brewery_name']] # and then reorder the columns to prioritize the most important info
beers = beers.drop_duplicates() # and then eliminate duplicates

# load the model and build the interaction matrix
model = joblib.load("final-model.joblib") #LightFMResizable object with the fitted model
...
def on_brewery_select(event):
    ...
    beers_for_brewery = beers[beers['brewery_name'] == selected_brewery]['beer_name'].tolist()
...
def on_brewery_search(event):
    ...
    with warnings.catch_warnings():
        warnings.filterwarnings("ignore", category=UserWarning, message=".*Applied processor reduces input query to empty string.*")
        
        # Perform fuzzy matching to find breweries that match the user's input
        matched_breweries = process.extract(query, beers['brewery_name'].unique(), limit=5)
...
def on_beer_select(event):
    ...
    selected_beer_ids.append(beers.loc[(beers.brewery_name==selected_brewery) & (beers.beer_name == selected_beer),'beer_id'].values[0])
    s = "Selected Combinations:" + str(selected_combinations)# + '\n' + "Selected Beer IDs:" + str(selected_beer_ids)
...
def on_beer_search(event):
    ...
    with warnings.catch_warnings():
        ...
        # Perform fuzzy matching to find breweries that match the user's input
        matched_beers = process.extract(query, beers.loc[beers.brewery_name == brewery, 'beer_name'].unique(), limit=10)
...
header_label = tk.Label(root, text= "Beer Recommendation instructions:",  
                        justify='center')
