...
def get_beers(data):
    ...
    beers = (
        beers.drop_duplicates()
    )  # and then eliminate EXACT duplicates just keeping the first instance of each
    return beers
...
def merge_similar_name_breweries(data):
    ...
    similar_names = [
        ...
        ["Hops Grillhouse & Brewery", "Hops Grill & Brewery"],
        ["Rock Bottom Restaurant & Brewery", "Rock Bottom Restaurant and Brewery"],
        ["Les Trois Brasseurs", "Les 3 Brasseurs"],
    ]
    for similar_name in similar_names:
        data.loc[
            data.brewery_name.isin(similar_name[1:]), "brewery_name"
        ] = similar_name[0]
    return data
...
def merge_brewery_ids(data):
    ...
    duplicates = breweries.brewery_name.value_counts().loc[
        breweries.brewery_name.value_counts() > 1
    ]
    ...
    for duplicate in duplicates.index:
        data.loc[data.brewery_name == duplicate, "brewery_id"] = breweries.loc[
            breweries.brewery_name == duplicate, "brewery_id"
        ].max()
    return data
